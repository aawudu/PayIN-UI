...
##else
    open -aXcode "dynamic-tab-bar.xcodeproj"
##endif
    exit $?
    ;;
uninstall)
    echo "Uninstalling com.apps.dynamictabbar"
    "/usr/local/share/uno/Packages/UnoCore/1.2.2/prebuilt/iOS/bin/ios-deploy" -9 -1 "com.apps.dynamictabbar"
    exit $?
    ;;
esac
...
##else
"/usr/local/share/uno/Packages/UnoCore/1.2.2/prebuilt/iOS/bin/ios-deploy" --noninteractive --debug --bundle "build/Release-iphoneos/dynamic-tab-bar.app" "$@"
##endif
